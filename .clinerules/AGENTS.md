...
在阅读本规则之前，请注意你应该已经读过**全局规则**（本机 Document 文件夹\Cline\Rules\java-coding-standards.md）了。
本文档仅补充其未涉及的部分。 

在开始任务之前，请阅读 **项目上下文文档**（项目根目录/contexts/CONTEXT.md）了解项目目前的情况。
在每次完成修改任务后，也应该及时更新 CONTEXT.md 的内容。
更新的时候不应该写实现的细枝末节，不要写“怎么做”，要写“做了什么”，具体实现应当在需要时去翻阅源码。
...
public class XxxAction extends Action {
    ...
    public void execute(/*...*/) {
        ...
        // 我们正在筹划改进它，详见 ROADMAP.md。若已经改进，需提醒用户更新本文。
    }
}
```
