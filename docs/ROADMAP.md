...
- [x] 完善 `README.md`
- [x] 完善 GitHub、模组发布页的文字、GitHub Release 文案、
- [x] 自己在游戏中测试体验功能，发现与修复 Bug、发现可改进项并安排添加时间
...
- [x] 实现检测项/保护动作特有的配置项。
- [x] 添加配置版本号，实现旧配置自动迁移逻辑。
- [ ] 完全自定义 HUD 覆盖层，加入自定义 HUD 元素，支持配置。

### 2.2 新增检测项

- [ ] **装备耐久检测** (`status/low_durability`)：轮询玩家盔甲栏位和手持物品耐久度，低于阈值时触发 → 自定义 HUD 提示 +
  对应槽位警告。
- [ ] **药效检测** (`status/effect_hud`)：监听 `player.getStatusEffects()`，在自定义 HUD 绘制当前 Buff/Debuff
  图标及剩余时间，时效不足时高亮警告。
- [ ] **光照检测** (`environment/anti_darkness`)：仅当在矿洞内计算亮度可生成怪物时 → 渲染红色警告；计算最佳放火把位置。

### 2.3 调整和扩展已有检测项的功能

- [ ] **防偷袭检测** 若有 Screen 处于开启状态，则前方来袭的威胁也要报警。
- [ ] **防摔落检测** 补充：计算目标位置与当前位置间是否会造成死亡或致命伤（摔落伤害预测）。
- [ ] **低血量检测** 补充：计算斩杀线。
- [ ] **岩浆检测** 补充：若已经落入岩浆，提醒玩家可以自救的物品。
- [ ] 合并 **低血量检测** 和 **低饥饿值检测**
- [ ] **弹射物追踪检测** 高亮手持可生成弹射物物品且面向玩家的实体。
- [ ] 将 `ActionBarTitleAction` 重构为抽象基类（`updateTitle()` 改 `protected`），各检测项以静态内部子类持有自己的
  文本组装逻辑（恢复 DangerLevel 引入前的分散式设计，用继承保留统一的样式/仲裁），使"检测项只管如何检测，保护
  动作只管如何处理"原则贯彻到底。可参考 2.1 中 `OutlineAction`/`BlockOutlineAction` 的抽象化改造作为先例。
...
- [ ] **弹射物追踪检测** 补充：渲染弹射物飞行弹道线（渲染实现可能较复杂）；主动躲避弹射物、打回火球。

### 3.2 结构性优化

- [ ] (可选 ?) 搞定构建系统和持续集成部署，解决多版本、多加载器兼容问题。
